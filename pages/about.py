...
with col2:
    ...
    KSAT Agent는 수능 국어 독서 영역 출제를 자동화하는 Multi-Agent 시스템입니다. 2년간 국어 모의고사 출제자로 활동한 경험과 노하우를 AI 시스템에 구현했습니다. 기존 출제 프로세스의 시간과 비용을 90% 이상 단축하면서도, 수능 기출 특유의 문체와 출제 코드를 담아냈습니다.
    """, unsafe_allow_html=True)
    ...
    |------|---------|--------------------|
    | **소요 시간** | 1 ~ 2 개월 | **~10 분** |
    | **비용** | 100~200만 원+ | **~1000 원** |
    | **의사소통** | 서면 피드백 반복 | AI와 실시간 대화 |

    - 기존 출제 프로세스 대비 비용과 출제 시간을 90% 이상 단축
    - 빠르고 간편한 국어 모의고사 콘텐츠 제작을 위한 출제 도구로 활용 가능
    ...
    AI에게 원하는 출제 분야나 구체적인 주제를 직접 입력합니다. 대화형 웹 인터페이스를 통해 직관적으로 요청을 전달할 수 있습니다.
    """, unsafe_allow_html=True)
    ...
    입력된 주제와 유사한 기출 지문을 검색하고, 개요 작성에 활용합니다. 이를 통해 기출 지문의 특성과 출제 의도를 명확히 파악할 수 있습니다.
    """, unsafe_allow_html=True)
    ...
    최신 정보나 학술적 주제에 대해 웹 검색 도구를 활용해 자료를 수집합니다. 신뢰할 수 있는 정보를 바탕으로 보다 정확한 지문을 작성할 수 있습니다.
    """, unsafe_allow_html=True)
    ...
    AI 에이전트가 참고 자료를 바탕으로 지문 개요를 설계하고, 파인튜닝된 전용 AI 모델이 실제 수능 스타일의 독서 지문을 생성합니다.
    """, unsafe_allow_html=True)
    ...
    지문이 작성되면 출제 AI 에이전트가 문항 출제에 필요한 기출 정보를 수집하고, 새로운 문항의 출제 방향을 설계합니다.
    """, unsafe_allow_html=True)
    ...
    AI가 생성한 지문과 문제, 해설을 한 눈에 확인하고, 입력창을 통해 추가 수정을 진행할 수 있습니다.
    """, unsafe_allow_html=True)
    ...
    with st.expander("일반 AI가 작성한 수능 지문", expanded=True):
        st.markdown("""
        <style>
    ...
    with st.expander("KSAT Agent가 작성한 수능 지문", expanded=True):
        st.markdown("""
        ### B. KSAT Agent 결과물
    ...
    기본 GPT 모델은 친절하고 쉽게 설명하도록 학습되어 수능 지문 특유의 압축적 정보 밀도를 구현할 수 없었습니다. 수능 독서 지문은 제한된 공간 안에 학술적이고 촘촘한 정보를 담아야 하는 독특한 문체적 특성을 가지고 있습니다. 따라서 파인튜닝을 통해 수능 독서 지문 특유의 문체와 정보 밀도를 구현할 수 있도록 전용 AI 모델을 학습시켰습니다.
